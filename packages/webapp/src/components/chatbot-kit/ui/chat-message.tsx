        ...
        {showSplit && !deterministic && typeof detPct === "number" ? (
          <div className="border-t border-border/40 bg-background/40 px-3 py-2 text-[11px] text-muted-foreground">
            ...
            <ul className="flex flex-col gap-0.5">
              ...
              {typeof modPct === "number" && modPct > 0 ? (
                <li>
                  <span className="font-mono">{modPct}%</span> generated by
            ...
            {typeof tokensUsed === "number" && tokensUsed > 0 ? (
              <div className="mt-1 text-[10px] italic text-muted-foreground/80">
                LLM usage: ~{tokensUsed.toLocaleString()} tokens (cumulative
                across turns — re-counts context re-sent each turn, so it
                overstates effort).
              </div>
            ) : null}
          </div>
